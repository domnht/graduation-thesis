...
\begin{longtable}{ScScScScSrSrSrSr}
	...
	$i$ & \textbf{Lựa chọn} & \textbf{Đáp án} & $u_i$ & \multicolumn{1}{c}{$a_i$} & \multicolumn{1}{c}{$b_i$} & \multicolumn{1}{c}{$\theta_{i+1}$} & \multicolumn{1}{c}{$SE(\theta_{i+1})$}\\\hline\endfirsthead

	$i$ & \textbf{Lựa chọn} & \textbf{Đáp án} & $u_i$ & \multicolumn{1}{c}{$a_i$} & \multicolumn{1}{c}{$b_i$} & \multicolumn{1}{c}{$\theta_{i+1}$} & \multicolumn{1}{c}{$SE(\theta_{i+1})$}\\\hline\endhead\hline\endfoot
...
Số câu hỏi là $k=1\leqslant 10$, kỳ vọng thay đổi năng lực $SE(\theta_2)\approx 0.93>0.8$, do đó các điều kiện dừng của trắc nghiệm thích ứng chưa được thỏa mãn. Kant tiếp tục với việc chọn câu hỏi thứ hai, sao cho phù hợp với mức năng lực $\theta_1$ và không lặp lại câu hỏi trước.\par
...
Học sinh S13 có điểm đánh giá qua Kant $9.5$ và điểm trung bình môn Toán là $9.0$, sai số tuyệt đối $0.5$ thuộc vào \textit{yếu vị} (mode) của thống kê. Trong quá trình đánh giá (bảng \ref{tab:tab-s4-result-of-s13}), học sinh trả lời đúng 8/10 câu hỏi. Đánh giá dừng sau khi thực hiện số lượng tối đa câu hỏi cho phép. Hệ số năng lực cũng đạt được giá trị ổn định ở 02 câu hỏi cuối, với $\theta_9\approx\theta_{10}\approx 1.78$.
